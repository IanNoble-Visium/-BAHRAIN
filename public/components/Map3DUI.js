...
function setupNavigationHandlers(viewer, panel) {
  ...
  // Tilt control - Directly modify camera pitch
  ...
  if (tiltControl && viewer) {
    tiltControl.addEventListener('input', (e) => {
      ...
      tiltValue.textContent = pitchDegrees + '°';

      // Calculate the pitch change needed
      const currentPitch = viewer.camera.pitch;
      const targetPitch = Cesium.Math.toRadians(pitchDegrees);
      const pitchDelta = targetPitch - currentPitch;

      // Apply the pitch change by rotating the camera
      viewer.camera.lookUp(pitchDelta);
    });
  }
